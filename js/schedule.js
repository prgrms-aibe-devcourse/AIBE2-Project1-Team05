...
function loadScheduleData() {
  ...
  try {
    ...
    console.error(
      "저장된 일정 데이터를 파싱하거나 표시하는 중 오류 발생:",
      error
    );
...
function displayDefaultScheduleMessage(message = "저장된 일정이 없습니다.") {
  ...
  if (metaElement)
    metaElement.textContent = "AI 추천 메뉴에서 일정을 저장해보세요.";
  if (sectionsContainer)
    sectionsContainer.innerHTML = `<p style="text-align: center; color: #a0a4b8; margin-top: 20px;">${message}</p>`;
}
...
function updateScheduleHeader(planData) {
  ...
  if (datesElement) {
    ...
    if (planData.selections && planData.selections.dateRange) {
      const [startDateStr, endDateStr] =
        planData.selections.dateRange.split(" to ");
      if (startDateStr && endDateStr) {
        try {
          ...
          if (!isNaN(startDate.getTime()) && !isNaN(endDate.getTime())) {
            ...
            console.warn(
              "날짜 형식이 잘못되었습니다(Date Range):",
              planData.selections.dateRange
            );
      ...
      try {
        ...
        if (!isNaN(startDate.getTime()) && !isNaN(endDate.getTime())) {
          ...
          const formatDate = (date) => {
            /* ... */
          }; // 위에 정의된 formatDate 사용
          dateRangeText = `${formatDate(startDate)} ~ ${formatDate(endDate)}`;
        } else {
          dateRangeText = `${planData.startDate} ~ ${planData.endDate} (형식?)`;
        }
      } catch (e) {
        dateRangeText = "날짜 처리 오류";
      }
    } else {
      // planData.days 길이를 기반으로 추정
      dateRangeText = `${planData.days.length - 1}박 ${
  ...
  if (metaElement) {
    ...
    if (planData.selections) {
      const themes =
        planData.selections.themes && planData.selections.themes.length > 0
...
function createScheduleSections(planData) {
  ...
  if (!sectionsContainer) {
    console.error(
      "ID가 'schedule-sections-container'인 요소를 찾을 수 없습니다."
    );
    return;
  }
        ...
        dayData.items.forEach((item, itemIndex) => {
          ...
          const imageTag = imageUrl
            ? `<img src="${imageUrl}" alt="${
                item.name || ""
              }" class="schedule-item-image" onerror="this.style.display='none'">` // onerror로 이미지 로드 실패 시 숨김
            : "";
...
function initScheduleMap() {
  ...
  let initialCenter = { lat: 37.5665, lng: 126.978 }; // 서울
  const planData = window.loadedPlanData; // loadScheduleData에서 전달받은 데이터

  if (
    planData &&
    planData.days.length > 0 &&
    planData.days[0].items.length > 0 &&
    typeof planData.days[0].items[0].lat === "number"
  ) {
    initialCenter = {
      lat: planData.days[0].items[0].lat,
      lng: planData.days[0].items[0].lng,
    };
  } else if (planData && planData.city) {
    // 도시 이름 기반으로 설정
...
function addMarkersAndRouteToScheduleMap(planData) {
  if (!scheduleMap || !google || !planData) {
    console.warn(
      "지도 객체 또는 planData가 준비되지 않아 마커를 추가할 수 없습니다."
    );
    return;
  }
  ...
  planData.days.forEach((dayData) => {
    ...
    dayData.items.forEach((item, itemIndex) => {
      if (typeof item.lat === "number" && typeof item.lng === "number") {
        ...
        const infoContent = `
                     <div style="color: #333;">
                         <strong>${dayData.day}-${itemIndex + 1}. ${
          item.name || ""
        }</strong><br>
                         ${
                         ...
                         ${
                           item.image
                             ? `<br><img src="${item.image}" alt="${item.name}" style="width:100px; margin-top:5px;" onerror="this.style.display='none'">`
                             : ""
                         }
                     </div>`;
